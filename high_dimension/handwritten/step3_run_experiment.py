...
on digit-8 images in the 100-dim NRAE latent space.
"""
import argparse
import json
import os
...
from config import (
    DATA_DIR, LATENT_DIR, CKPT_DIR, RESULTS_DIR,
...
from metrics       import (
    extract_features, compute_tau,
    bootstrap_metrics,
)

# MM-SOLD sampler from the shared augmentation_handwritten module
...
def _decode(model, params, Z: np.ndarray, n_dct: int) -> np.ndarray:
    """Decode NRAE latents → (N, 64, 64) float32 images."""
...
def _measure_cpu_time(fn, *args, n_warmup=1, n_timed=3, **kwargs):
    """
    Run fn(*args, **kwargs) on CPU, warm up then average over n_timed runs.
    Forces JAX computation to CPU by temporarily placing ops there.
    Returns average wall-clock time in seconds.
    """
...
def main(args):
    ...
    print(f"Total cells : {len(SIGMA_GRID) * len(M_GRID)} per method\n")
    ...
    # ── Extract test features (fixed reference) ────────────────────────────
    ...
    print(f"  DupRate τ = {tau:.5f}  (5th pct within-train NN dist)")

    # ── Compute whitening stats (shared across grid) ───────────────────────
    print("\nComputing whitening stats ...")
    mean_cls, S_sqrt_cls, S_invsqrt_cls = _whiten_stats(
        Z_train, k=SOLD_K_WHITEN)
    print("  Done.")

    # ── Generate DDPM samples (once, reused for all grid cells) ───────────
    print(f"\n{'='*40}")
    ...
    # measure DDPM sampling time on default device
    rng, k_ddpm = jax.random.split(rng)
    print(f"  Timing DDPM inference ({DDIM_STEPS} DDIM steps, {N_GENERATE} samples)...")
    t0 = time.perf_counter()
    ...
    if whiten_mu is not None and whiten_std is not None:
        ...
        print(f"  Un-normalised DDPM latents → Z_gen_ddpm: {Z_gen_ddpm.shape}")

    print(f"  Decoding {N_GENERATE} DDPM latents ...")
    imgs_gen_ddpm = _decode(nrae_model, nrae_params, Z_gen_ddpm, n_dct)
    print(f"  imgs_gen_ddpm: {imgs_gen_ddpm.shape}")

    print("  Extracting DDPM CNN features ...")
    ...
    # Results: indexed by [sigma_idx][M_idx]
    grid_sold = [[None] * len(M_GRID) for _ in range(len(SIGMA_GRID))]
    grid_cfdm = [[None] * len(M_GRID) for _ in range(len(SIGMA_GRID))]
    ...
    for si, sigma in enumerate(SIGMA_GRID):
        for mi, M in enumerate(M_GRID):
            ...
            # Timing: measure on CPU with JIT warm-up
            if measure_timing:
            ...
            if args.save_latents:
                np.save(
                    os.path.join(args.results_dir,
                                 f"Z_gen_sold_{si}_{mi}.npy"), Z_sold)
                np.save(
                    os.path.join(args.results_dir,
                                 f"Z_gen_cfdm_{si}_{mi}.npy"), Z_cfdm)
    ...
    timing = {
        ...
        "sold_cpu_time_per_sample_ms":     timing_sold / N_GENERATE * 1000 if timing_sold is not None else None,
        "cfdm_cpu_time_s":                 timing_cfdm,
        "cfdm_cpu_time_per_sample_ms":     timing_cfdm / N_GENERATE * 1000 if timing_cfdm is not None else None,
    ...
    # merge with DDPM training time if available
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_latents", action="store_true",
                        help="Save generated latents to disk (large)")
