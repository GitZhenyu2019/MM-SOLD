...
from config import (
    ...
    SOLD_K_WHITEN, SOLD_NSTEPS, SOLD_H, SOLD_SIGMA_GMM, SOLD_DISCRETIZ,
...
def _heatmap_relative(arr_sold, arr_cfdm, better, out_path,
                      sigma_grid, m_grid, fmt="{:+.3f}"):
    ...
    ax.set_xlabel("σ", fontsize=9); ax.set_ylabel("M", fontsize=9)
...
def _pick_vis_nn(arr, Z_ref, n, seed=VIS_SEED, lo_pct=10, hi_pct=75):
    q2    = np.sum(arr    ** 2, axis=1, keepdims=True)
    r2    = np.sum(Z_ref  ** 2, axis=1, keepdims=True)
    cross = arr @ Z_ref.T
    dist2 = np.maximum(q2 + r2.T - 2.0 * cross, 0.0)
    nn_dist = np.sqrt(dist2.min(axis=1))
    lo = np.percentile(nn_dist, lo_pct)
    hi = np.percentile(nn_dist, hi_pct)
    mask  = (nn_dist >= lo) & (nn_dist <= hi)
    cands = arr[mask]
    if len(cands) == 0:
        cands = arr 
    if len(cands) <= n:
        return cands
    idx = np.random.default_rng(seed).choice(len(cands), size=n, replace=False)
    return cands[idx]


def _save_seamless_grid_rgb(images, path):
    G = VIS_GRID
...
def _save_nn_vis_rgb(query_imgs, nn_imgs, path, n_cols=4, n_rows=8, gap=4):
    ...
    for k in range(n_cols * n_rows):
        ...
        y0 = row * (H + gap); x0 = col * (pair_w + gap)
...
def main(args):
    ...
    for key, name, better, label in metrics_cfg:
        grids[key] = {
            ...
            "better": better, "label": label, "name": name,
        }
    ...
    ax.set_title(
        f"Summary [KNN K={knn_K} L={knn_L}]  "
        f"(mean±std, {results.get('score_estimator','knn_remainder')})",
        fontsize=11, pad=8)
    plt.tight_layout()
    ...
    if timing:
        ...
        if labels:
            ...
            ax.set_title(
                ...
                f"N={timing.get('n_generate','?')} samples", fontsize=10)
    ...
    if args.make_grids:
        ...
        if args.select_by == "nn_dist":
            print("  Selection mode: nn_dist  (lo_pct=10, hi_pct=75)")
            def pick_fn(arr, n): return _pick_vis_nn(arr, Z_train, n)
        else:
            print("  Selection mode: random")
            def pick_fn(arr, n): return _pick_vis(arr, n)
        ...
        if os.path.exists(ddpm_ckpt_path):
            ...
            if os.path.exists(ddpm_lat_path):
                ...
                if _wtype == "pca_zscore":
                    ...
                    if all(x is not None for x in [_pca_mean, _pca_comps, _pca_std]):
                        Z_ddpm = (np.array(Z_ddpm) * _pca_std @ _pca_comps + _pca_mean).astype(np.float32)
                elif _wtype == "pca":
            ...
            Z_ddpm_pool    = pick_fn(Z_ddpm, N_VIS)
            ...
            imgs_nn_d = _decode_imgs(Z_train[nn_idx_d])
            _save_nn_vis_rgb(imgs_q_ddpm, imgs_nn_d,
                             os.path.join(grid_fig_dir, "nn_ddpm.png"))
            print(f"  DDPM  avg NN dist: {np.mean(nd):.4f}")

        # QR retraction requires n_particles >= latent_dim (700 for CelebA).
        # VIS_GRID²=64 < 700 → use more particles and subsample.
        _N_GEN_GRID = max(N_VIS * 4, 800)
        ...
        for si, sigma in enumerate(sigma_grid):
            for mi, M in enumerate(m_grid):
                ...
                Z_cfdm_pool    = pick_fn(Z_vis_cfdm, N_VIS)
                ...
                imgs_nn_c = _decode_imgs(Z_train[nn_ci])
                _save_nn_vis_rgb(
                    imgs_cfdm_pool[:N_NN], imgs_nn_c,
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Step 4 (KNN+remainder): plot CelebA-HQ-256 results.")
    ...
    parser.add_argument("--select_by",   type=str, default="random",
                        choices=["random", "nn_dist"],
                        help="How to pick images for grids: "
                             "'random' (default) or 'nn_dist' (filter by "
                             "nearest-neighbour distance to training set, "
                             "lo_pct=10 hi_pct=75)")
    ...
    if args.fig_dir is None:
        args.fig_dir = os.path.join(
            here, f"figures_knn_K{args.knn_K}_L{args.knn_L}")
