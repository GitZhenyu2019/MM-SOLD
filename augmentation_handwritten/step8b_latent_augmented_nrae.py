"""
step8b_latent_augmented_nrae.py
================================
Train MLP classifier on AUGMENTED NRAE latent vectors
(original Z_train + MM-SOLD generated latents from step6b).
Grid search over weight_decay; model selected by best validation accuracy.
Compares final test accuracy with the NRAE baseline from Step 7b.
"""
...
def main(args):
    print("=" * 60)
    print("Step 8b: NRAE Latent-Space Augmented Classifier")
    print("=" * 60)
    print(f"JAX backend: {jax.default_backend()}  devices: {jax.devices()}")

    # ---- Load augmented latents (shared val/test) ----
    d = args.latent_dir
    ...
    print(f"\nAug-Train: {Z_train.shape}  ({n_orig} orig + {n_gen} generated)")
    ...
    aug_acc = evaluate_mlp_verbose(
        ...
        hidden_dim=args.hidden_dim,
        tag="nrae latent augmented")

    # ---- Save checkpoint ----
    os.makedirs(args.ckpt_dir, exist_ok=True)
    ckpt = os.path.join(args.ckpt_dir, "mlp_nrae_augmented_best.pkl")
    ...
    # ---- Compare with step7b baseline ----
    baseline_ckpt = os.path.join(args.ckpt_dir, "mlp_nrae_baseline_best.pkl")
    if os.path.exists(baseline_ckpt):
        ...
        if base_acc is not None:
            ...
            print(f"\n{'='*60}")
            print(f"  NRAE latent baseline  : {base_acc:.2f}%")
            print(f"  NRAE latent augmented : {aug_acc:.2f}%")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Step 8b: NRAE latent-space augmented MLP with grid search.")
