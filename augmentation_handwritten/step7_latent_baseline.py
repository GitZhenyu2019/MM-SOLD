"""
step7_latent_baseline.py
========================
Train MLP classifier on ORIGINAL VAE latent vectors (Z_train).
Grid search over weight_decay; model selected by best validation accuracy.
Reports per-class and overall test accuracy.
"""
...
from classifier_model import save_params


def main(args):
    print("=" * 60)
    print("Step 7: Latent-Space Baseline Classifier (original latents)")
    print("=" * 60)
    print(f"JAX backend: {jax.default_backend()}  devices: {jax.devices()}")

    # ---- Load latents ----
    d = args.latent_dir
    Z_train = np.load(os.path.join(d, "Z_train.npy"))
    ...
    print(f"\nTrain: {Z_train.shape}  Val: {Z_val.shape}  Test: {Z_test.shape}")
    ...
    test_acc = evaluate_mlp_verbose(
        best_params, Z_test, y_test,
        hidden_dim=args.hidden_dim,
        tag="latent baseline")

    # Save best params + metadata (step 8 reads best_test_acc for comparison)
    os.makedirs(args.ckpt_dir, exist_ok=True)
    ckpt = os.path.join(args.ckpt_dir, "mlp_baseline_best.pkl")
    save_params(ckpt, best_params,
                info={"best_val_acc": best_val, "best_test_acc": test_acc,
                      "best_wd": best_wd, "best_epoch": best_epoch})
    print(f"\nCheckpoint saved: {ckpt}")
    print(f"\nStep 7 complete!  Test accuracy (latent baseline): {test_acc:.2f}%")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Step 7: Latent-space baseline MLP with grid search.")
